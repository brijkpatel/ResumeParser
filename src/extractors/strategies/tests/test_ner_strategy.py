...
class TestNERExtractionStrategy:
    ...
    def test_init_with_invalid_model(self, mock_gliner):
        ...
        with pytest.raises(InvalidStrategyConfigError):
            NERExtractionStrategy(model_name="invalid_model_xyz")

    @patch("extractors.strategies.ner.GLiNER.from_pretrained")
    def test_init_with_valid_model(self, mock_gliner):
        """Test successful initialization."""
        mock_model = Mock()
        mock_gliner.return_value = mock_model

        strategy = NERExtractionStrategy()
        assert strategy.model == mock_model
        mock_gliner.assert_called_once_with("urchade/gliner_multi_pii-v1")

    @patch("extractors.strategies.ner.GLiNER.from_pretrained")
    def test_extract_single_entity(self, mock_gliner):
        ...
        strategy = NERExtractionStrategy(default_entity_labels=["person"])
        ...
        mock_model.predict_entities.assert_called_once_with("My name is John Doe", ["person"])

    @patch("extractors.strategies.ner.GLiNER.from_pretrained")
    def test_extract_multiple_entities_with_top_k(self, mock_gliner):
        ...
        result = strategy.extract("I know Python Java JavaScript Ruby", spec)
        assert isinstance(result, list)
        assert len(result) == 2
        assert result == ["Python", "Java"]

    @patch("extractors.strategies.ner.GLiNER.from_pretrained")
    def test_extract_no_entity_label_raises_error(self, mock_gliner):
    ...
    @patch("extractors.strategies.ner.GLiNER.from_pretrained")
    def test_extract_empty_text_raises_error(self, mock_gliner):
    ...
    @patch("extractors.strategies.ner.GLiNER.from_pretrained")
    def test_extract_no_entities_found_raises_error(self, mock_gliner):
        ...
        with pytest.raises(NoMatchFoundError, match="No entities found"):
            strategy.extract("Some text without entities", spec)

    @patch("extractors.strategies.ner.GLiNER.from_pretrained")
    def test_extract_gliner_processing_error(self, mock_gliner):
    ...
    @patch("extractors.strategies.ner.GLiNER.from_pretrained")
    def test_extract_with_top_k_zero_returns_all(self, mock_gliner):
        ...
        result = strategy.extract("Alice Bob Charlie", spec)
        assert len(result) == 3
